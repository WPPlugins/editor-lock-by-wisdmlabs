...
function elbw_baw_settings_page()
{
    ...
    $elbw_fetch_database = $wpdb->get_var('SELECT * FROM `wp_usermeta` ORDER BY `user_id` DESC', 1);
    while ($elbw_fetch_database >= 2)
    {
	$elbw_search_admins = get_user_meta($elbw_fetch_database, 'wp_user_level', true);
